...
def startt(request,num):
    fu = Odai.objects.get(id=num)
    fu_1 = Odai.objects.all().get(id=num)
    datas = Follow.objects.filter(user=request.user).values_list('follow')
...
def whogood(request):
    #いいねしたゲームを取得
    data = Goodgame.objects.filter(user=request.user).values_list('good__odai',flat = True)
    datas = Odai.objects.filter(odai__in = data)

    # datas = Odai.objects.filter(odai_in=data)
